...
class Sequelize {
  ...
  normalizeAttribute(attribute) {
    ...
    if (attribute.type instanceof DataTypes.SET) {
      // The SET is a special case where the type is an object containing the values
      if (attribute.values) {
        attribute.type.values = attribute.type.options.values = attribute.values;
      } else {
        attribute.values = attribute.type.values;
      ...
      if (!attribute.values.length) {
        throw new Error('Values for SET have not been defined.');
      }
    }
